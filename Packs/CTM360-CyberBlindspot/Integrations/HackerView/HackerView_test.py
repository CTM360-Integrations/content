    ...
    [
        (
            ...
            {"ticketId": "HVI-11145070"},
            {
                ...
                "hackerview_link": "https://hackerview.ctm360.com/issues/issues_found/overview/"
                "56e192172413d0bbcf7aa2f82bfb102f/20221292",
                "member_id": 73069,
                ...
                "port": 443,
                "asset_type": "ip",
                "scan_id": "97a186944e5c708855688784478980c0",
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"issueId": "HVI-TEST", "issueStatus": "investigating", "comment": "test"},
...
@pytest.mark.parametrize(
    "mock_status,mock_progress",
    [
        ("active", "Investigating"),
        ("active", "In Progress"),
        ("active", "Fixed"),
        ("active", "Acceptable Risk"),
        ("active", "False Positive"),
        ("inactive", "New"),
        # ("inactive", "Investigating"),
        # ("inactive", "In Progress"),
        # ("inactive", "Fixed"),
        # ("inactive", "Acceptable Risk"),
        # ("inactive", "False Positive"),
    ],
)
def test_get_remote_data(mock_status, mock_progress, mock_client, mocker, monkeypatch):
    """
    Given:
        - HackerView Client.
        - Client arguments.
    ...
    from HackerView import get_remote_data_command, map_and_create_incident
    ...
    mock_result["progress_status"] = mock_progress

    def demisto_incident():
        return map_and_create_incident(load_mock_response("incident_details_response_valid.json")[0])
    mocker.patch.object(mock_client, 'fetch_incident', return_value=deepcopy(mock_result))
    monkeypatch.setattr('demistomock.incident', demisto_incident)

    mock_result2 = map_and_create_incident(mock_result) if mock_status and isinstance(mock_result, dict) else []
