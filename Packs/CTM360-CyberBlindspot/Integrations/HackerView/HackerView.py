...
HV_INCIDENT_FIELDS = [
    ...
    {'name': 'issue_id', 'description': 'Numerical Incident ID.'},
    {'name': 'issue_name', 'description': 'Name of Incident.'},
    {'name': 'issue_type', 'description': 'Type of Incident.'},
    ...
    {'name': 'priority', 'description': 'Incident Response Priority.'},
    ...
    {'name': 'fixing_effort', 'description': 'Amount of effort required to fix.'},
    # {'name': 'hackerview_link', 'description': 'External link to incident on remote platform.'},
    {'name': 'member_id', 'description': 'metadata attribute'},
    {'name': 'resolved_ip', 'description': 'IP resolved on affected asset.'},
    {'name': 'first_seen', 'description': 'Incident creation date.'},
    {'name': 'last_seen', 'description': 'Last update date for incident.'},
    {'name': 'environments', 'description': 'env.'},
    {'name': 'ticket_id', 'description': 'Ticket ID'},
    {'name': 'business_unit', 'description': 'metadata attribute.'},
    {'name': 'host_type', 'description': 'Affected host type.'},
    {'name': 'technologies', 'description': 'Technologies on affected asset.'},
    {'name': 'discovery_source', 'description': 'Source of data'},
...
def map_and_create_incident(unmapped_incident: dict) -> dict:
    ...
    unmapped_incident.pop('scan_id', '')
    unmapped_incident.pop('priority', '')
    unmapped_incident.pop('issue_id', '')
    unmapped_incident.pop('host_type', '')
    unmapped_incident.pop('issue_type', '')
    unmapped_incident.pop('member_id', '')
    unmapped_incident.pop('ticket_id', '')
    unmapped_incident.pop('fixing_effort', '')
    unmapped_incident.pop('business_unit', '')
    unmapped_incident.pop('hackerview_link', '')
    unmapped_incident.pop('discovery_source', '')
    mapped_incident = {
...
def get_remote_data_command(client: Client, args: dict) -> GetRemoteDataResponse:
    ...
    old_status = demisto.incident().get('external_status', '').lower()

    log(INFO, f'Updating incident {remote_incident_id} with status: {new_status}')
    ...
    if old_status == 'inactive' and new_status == 'active':
        ...
        close_reason = f'Incident was {"resolved" if new_status == "fixed" else "closed"}.'
        entries.append({
            'Type': EntryType.NOTE,
            'Contents': {
    ...
    elif new_status == 'active' and progress_status == 'investigating':
    ...
    elif new_status == 'active' and progress_status == 'in progress':
        ...
        entries.append(entry | {'Contents': note})
    elif new_status == 'active' and progress_status == 'fixed':
        ...
        note = 'Your team has implemented all necessary changes and fixes to address this issue.\
             Our system will automatically label\
             issues as resolved once they are no longer present.'
        entries.append(entry | {'Contents': note})
    elif new_status == 'active' and progress_status == 'acceptable risk':
        ...
        entries.append(entry | {'Contents': note})
    elif new_status == 'active' and progress_status == 'false positive':
        log(DEBUG, 'Incident seems to be have ongoing monitoring. Adding monitoring entry')
...
def ctm360_hv_incident_status_change_command(
    ...
    log(
        INFO,
        f'Request to change status of incident {args["issueId"]}'
